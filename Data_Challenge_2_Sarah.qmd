...
ggplot() +
  ...
  geom_sf(data = heigo_graph |> activate(nodes) |> 
          filter(id %in% c(wiedikon_bhf_vertex, EuropaalleeHB_vertex)) 
          |> as_tibble() |> st_as_sf(), size = 2, col = 'orange') +
  
  # Add labels for the origin and destination
  geom_sf_text(data = heigo_graph |> activate(nodes) |> 
                 filter(id %in% c(wiedikon_bhf_vertex, EuropaalleeHB_vertex)) 
          |> as_tibble() |> st_as_sf(), aes(label = long_name),  nudge_x = 1500, size = 3) + 
  # Remove axis titles
  theme_void()
```
