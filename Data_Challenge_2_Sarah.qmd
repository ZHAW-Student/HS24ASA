...
# Introduction 
This file includes the assignments, code and interpretation of Data Challenge 2. The code was elaborated as collaboration between Sarah Wirth and Saskia Gianola. Each person has invested about 25 hours in this project. ChatGPT has been used to create and adapt code for this data challenge, but not for the interpretation of the results. 
...
I have never cycled in Zürich as I do not live here and consider lots of spaces to be rather dangerous for pedestrians and cyclists based on the aggressive driving style of some people. But I know the area from Wiedikon to the main station well and can try to give feedback on this.
...
The path chosen here does not correspond to the shortest path I would have taken for several reasons. Firstly, this path does take a rather large detour via Zürich Selnau instead of using the road via Werd. This could be due to the placement of the starting point or maybe the chosen algorithm causes this behavior. For the fastest route I would have used Werd and as my personal safety route I would have chosen a route using some small streets with little traffic. 
...
For this measure I assume, that cyclists do not avoid traffic hotspots, which I would absolutely do if I would have to cycle in Zürich. It also assumes that the speed of cyclists also is not affected by the slope of each path or speed limits. Also, we might need to transform the network as the betweenness centrality is calculated for the nodes. Other potential problems are that only the shortest path does only show the optimum but not other routes which are similarly suitable.
...
ggplot() +
  ...
  scale_color_viridis_c(option = "turbo", name = "betweenness centrality (nodes)") +
  theme_void()
```
...
ggplot() +
  ...
  scale_color_viridis_c(option = "turbo", name = "betweenness centrality (edges)") +
  theme_void()
```
:::

Although it is rather difficult to identify anything at the scale of the whole city some details are recognizable. Several nodes in the city center (Kreis 1) and the nodes of the Bucheggstrasse have an elevated betweenness centrality compared to most other nodes. For the edges a clearer picture is present. Firstly, the Hardbrücke and the Rosengartenstrasse as well as the Bucheggstrasse have a strongly elevated betweenness centrality compared to the rest of Zürich. Also, some edges along the Limmat like the Limmatquai are highlighted as well as the Sihlhochstrasse. All those roads are well known for having rather high traffic. Therefore, the betweenness centrality highlights areas with high traffic well.
...
The tunnel is already present in the dataset although it does not end exactly at one of the sets of the given coordinates.
...
The project strongly increases the betweenness centrality of the nodes at the Drahtschmidlisteg across the Limmat, the Sihlquai as well as the Kasernenstrasse on the southern side of the train station. It is also apparent, that the betweenness centrality of all nodes eastern of the train station is a little decreased by the addition of the Stadttunnel. Looking at the edges the same trend is visible. But it is also easily visible how the betweenness centrality on the southern side of the train station at the Poststrasse and the Gessnerallee is reduced through the addition of the Stadttunnel. Therefore the Stadttunnel should overall reduce bicycle presence in many rather dangerous areas surrounding the train station which could lead to a lower number of accidents involving cyclists in this region.
...
Nearby according to me has to respect the width of each road, and that each road in the grid is represented by a line which is exactly following the mid of the width of the road. Then we only need to respect the width of roads on which bicycles should be used (we exclude Autostrasse and Autobahn). As I cannot account for the width of every single road I will orient it on the largest road type bicycles can ride on (1. Klasse Strasse) which has a width of at least 6m. Therefore, I choose a distance of 8m, which contains enough space to include half of the road (3m), the pedestrian path (2m) and enough space for the trees to grow (3m). As the computers of Saskia and I do not have enough computational power to join for the area of the whole city, we chose to focus on the on Kreis 1, 4 and 5. This focus will be kept for the rest of DC2.

To prevent double counting all trees outside of the 8m buffer around the edges were dropped in the first step, then for all remaining trees the closest present edge was found, and the ID of the nearest edge was assigned to each tree. For this step a unique id for each edge was created. Then all trees per ID were counted and this count was joined back to the edges of the network.
...
# create buffer around edges, 8 m buffer = 3 m street + 2 m sidewalk, + 3 m buffer
edge_buffer <- st_buffer(edges_sf_clipped, dist = 8)

#clip trees to buffer
trees_clipped <- st_intersection(edge_buffer, trees_clipped)
...
## linear regression model where betweenness centrality predicts traffic
```{r}
...
ggplot(joined_data) +
  ...
  theme_minimal()
```
Looking at the plot a rather big amount of scattering is present. Without the point with a value of 4000 for the traffic per day and the highest betweenness centrality a declining curve could be seen. Unfortunately, the traffic was often measured in areas with a low betweenness centrality in our network, therefore a linear model will not fit but maybe transforming the betweenness centrality could help.

## linear regression model where log betweenness centrality predicts traffic
```{r}
...
Looking at the summaries and the RMSE of both models shows that transforming the betweenness centrality slightly improves the model (slightly lower RMSE, slightly bigger multiple R-squared). But the models remain not significant and are not suited to represent the relationship between the traffic per day and the betweenness centrality.
...
ggplot(joined_data) +
  ...
  theme_minimal()
```
In this case  only a small number of traffic- count locations have trees in the close-range buffer according the Baumkataster. Therefore, building a model is already not suited for only 11 points.
...
```
This plot already looks way better, but still lacks data.
...
# both approaches are not useful to predict traffic
```
Among the tree-count based models the log-transformed performs worse (higher RMSE, lower multiple R-squared). But in the end those differences do not matter a lot as both models are not significant, and no significant effect of the trees is apparent. The same goes for the comparison between the models based on the betweenness centrality and the models based on the tree counts. The models based on the tree counts are a little better, but all models are bad to a degree, where they should not be applied, especially the tree-based models solely based on their lack of data points.
:::
...
Looking at the data table the results show similar results to what was discussed previously in task 9. In the rest of the DC2 a model will be considered to be better than another one, if it has an AIC or BIC which is 2 AIC/BIC lower than another one, if the difference is smaller both models will be considered to be equally suited. Therefore, the tree based models outperform the betweenness centrality based models. And the log-transformation of the explanatory variables did not lead to an improvement of the model.
...
For the rest of the DC2 following changes will be applied: The NAs within the column with the number of accidents with the number 0, as we assume that all accidents in Zürich have been properly reported. Also, all cases with a NA for the betweenness centrality were excluded as well. Those could have been created by improperly connected roads in the network. We first thought about removing all cases where trees had NAs as well, but we did not as the Baumkataster only contains trees owned by the city but not private trees. Therefore, we cannot be sure that no trees are present in the cases with NAs, which led to us keeping the NAs for the trees. The cleanup was required to ensure the code to run properly without errors.
...
This summary contains a small but highly significant influence of the betweenness centrality. The residual deviance is a little smaller than the null deviance indicating that the model is slightly better. The AIC with a value of 17203 is a lot higher than the AIC of all previous models.

## Plot of probability of accidents
```{r}
...
The predicted probability represents the accident hotspots from task 6 pretty accurate and hotspots like the parts of the Limmatquai, the roads adjacent on the northern side of the Limmat and the Hardbrücke as well as the roads leading off it in northern and southwestern direction from the Hardbrücke. Although the Stadttunnel is not in use yet the high betweenness centrality of it and its adjacent roads lead to a high predicted probability that an accident occurs. This also highlights that the betweenness centrality alone is not enough to represent the risk of accidents occurring. Therefore, maybe adding other predictors or account for spatial components could improve the model.
...
Although it might seem that the accident count drops with increasing betweenness centrality and tree count the high count of low numbers of accidents could also show another trend.
...
plot(lm_betweenness)
```
Looking at the number of accidents plotted against the betweenness centrality again a linear model seems to be a bad fit. The plot of the residuals against the fitted values shows a wedge open to the left, which indicates a violation of variance homogeneity. Also, the deviation of the points from the dashed line in the Q-Q plot indicates a violation of the normal distribution of the residues.
...
Ploting the accidents against the number of trees again highlights how unsuited a linear model is for this case. The same problems as for the previous model is present.


## lm betweenness centrality and trees
```{r}
...
Looking at the comparison of the models the AIC and BIC show similar results. The best model is the linear model dependent on the betweenness centrality and the trees. Second best is the linear model dependent on trees followed by the poisson model and the model only relying on the betweenness centrality. Those results indicate that both predictors are capable of improving a model for the number of accidents. It is to assume that most of the predictive power stems from the tree count and the betweenness centrality only has a smaller influence.
...
The Moran's I shows some but not a lot of positive and highly significant (p < 0.0001) autocorrelation in the strongest model (lm_betweenness_trees). Therefore, spatial autocorrelation should be accounted for in further models to improve them
...
This model shows that both the betweenness centrality and the number of trees have a highly significant (p < 0.001) influence on the number of accidents even if the influence is very small (estimates in the hundredths) with a higher influence of the tree count. The influence of the disturbances in neighboring locations are significant (p value of likelihood ratio test = 0.01).
...
For this model comparison the BIC and AIC show slightly dissimilar results. Whereas the BIC does indicate no improvement through the use of the spatial error model the AIC shows slight improvement. Therefore accounting for the spatial autocorrelation does only improve the model slightly.
...
A highly significant (p < 0.001) but small effect from the scaled count of trees is present in the model but no effect from the scaled betweenness centrality (p = 0.18). 
There also is a small negative (-0.122) and therefore negligible correlation between the scaled betweenness centrality and the scaled count of trees.

## Anova linear mixed model
```{r message=TRUE}
anova(lmem)
```

This model is highly significant compared to a model without fixed effects and accounts for random effects of different highway types. Again, no significant effect of the scaled betweenness centrality is present.
:::
...
The mixed model (dependent on count of trees and betweenness centrality with highway as random effect) has the lowest BIC and AIC of all models and therefore is the best of the here listed models. Even if the random intercept model and the random model have similar BIC and AIC the random intercept model (dependent on betweenness centrality with highway as random effect) is slightly better than the random model. Worst of all models is the fixed model. 
