...
Saskia:These plots are used to test whether the requirements for a linear model are fulfilled. There is no variance homogeneity. The residuals are not normally distributed (top right). These results are not surprising as the data was not transformed. 
...
plot(lm_trees)
```
Saskia:The plots show a very similar picture as for the lm betweenness. The residuals are not normally distributed. 
...
plot(lm_betweenness_trees)
```
Saskia:Again we have very similar plots as in the two prevous cases. 
:::
...
morans_I_residuals
```
Saskia:The results from the Morans I shows a small positive autocorrelation of the data. The observed value is greater than the expected value if there was no autocorrelation. The p value is very low and there is significant autocorrelation. 
