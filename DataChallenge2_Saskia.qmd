...
# Ergebnis anzeigen
head(joined_data_acc[c("btw", "lambda", "p_no_accident")])
...
residuals_lm_btw <- residuals(lm_betweenness) |>
  as_tibble() |>
  setNames("residuals") |>
  mutate(uniqueID = joined_data_acc$uniqueID, 
         binned_residuals = cut_interval(residuals, n = 8))

edge_point <- st_centroid(joined_data_acc)

# Join with the polygons of upstate New York
joined_data_acc <- full_join(edge_point, 
                             residuals_lm_btw, by = "uniqueID")


# Plot the residuals --- plot noch nicht farbig
ggplot(joined_data_acc) +
...
joined_data_acc_knear <- knearneigh(joined_data_acc, k = 2)
joined_data_acc_nb <- knn2nb(joined_data_acc_knear)

# Binary spatial weights
joined_data_acc_weights <- nb2listw(nb, style = "B")

# Moran's I 
morans_I_residuals <- lm.morantest(lm_betweenness, listw = joined_data_acc_weights,
                                   alternative = "greater")
...
lag_joined <- lagsarlm (formula = count_acc ~ btw + count_trees, 
                           data = joined_data_acc, listw = joined_data_acc_weights)
