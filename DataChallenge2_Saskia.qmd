...
edge_buffer <- st_buffer(edges_sf, dist = 8)

# connect trees with buffered edges
trees <- st_join(edge_buffer, baumkataster_shp, left = FALSE)
...
accident_points <- st_join(edge_buffer,bike_accidents, join = st_nearest_feature)

# summarise to get amount of accidents per edge
...
joined_data <- st_join(biking_network_edges_sf, bike_counts, left = TRUE, largest = TRUE)
...
# clean data 
colnames(accident_per_joined_edge)
accident_per_joined_edge$btw[accident_per_joined_edge$btw == 0] <- NA
accident_per_joined_edge$highway.x <- factor(accident_per_joined_edge$highway.x)
levels(accident_per_joined_edge$highway.x)
...
linear <- lm(per_day ~ btw, data = accident_per_joined_edge) # y ~ x
...
ggplot(accident_per_joined_edge) +
...
loglinear <- lm(per_day ~ log(btw), data = accident_per_joined_edge)
...
loglinear_res <- accident_per_joined_edge |>   
...
categorical <- lm(per_day ~ highway.x + 0, data = accident_per_joined_edge)
rmse_categorical <- sqrt(mean(residuals(categorical)^2)) 

# Plot the mean traffic per highway type 
ggplot(accident_per_joined_edge, aes(x = highway.x, y = per_day)) +
  geom_boxplot() +
...
multivariate <- lm(per_day ~ log(btw) + highway.x, data = accident_per_joined_edge)
...
multivariate_bs <- c(multivariate_b, 
                     ...
                     multivariate_b + multivariate$coefficients["highwayservice"],
                     multivariate_b + multivariate$coefficients["highwaytertiary"])

# Plot the regression lines 
ggplot(accident_per_joined_edge) +
  geom_point(aes(x = btw, y = per_day, colour = highway)) +
  geom_line(
    data = data.frame(x = seq(300, 50000, by = 100)),
    aes(x = x, y = multivariate_m * log(x) + multivariate_bs[1], color = "highwayresidential")) +
  geom_line(
    data = data.frame(x = seq(300, 50000, by = 100)),
    aes(x = x, y = multivariate_m * log(x) + multivariate_bs[2], color = "highwayservice")) +
  geom_line(
    data = data.frame(x = seq(300, 50000, by = 100)),
    aes(x = x, y = multivariate_m * log(x) + multivariate_bs[3], color = "highwaytertiary")) +
  scale_x_continuous(trans = "log", breaks = c(400,4000,40000)) +
...
# extreme <- lm(per_day ~   + log(btw) + highway * poly(log(nt), 2) + 
...
# compute glm
model_glm <- glm(per_day ~ btw, data = edges_join, family = binomial)
summary(model_glm)

intercept_glm <- model_glm$coefficients[[1]]
slope_glm <- model_glm$coefficients[[2]]

ggplot(data = edges_join) +
  geom_point(aes(x = btw, y = per_day)) +
  geom_line(data = data.frame(x = seq(0, 600, by = 0.1)),
            aes(x = x, y = 1/(1 + exp(- intercept_glm - slope_glm * x))),
            color = "red",
            linewidth = 1.5) +
  theme_minimal()
...
```{r}
poisson_model <- glm(count ~ btw, data = accident_per_joined_edge, family = "poisson")
...
ggplot(accident_per_joined_edge, aes(x = btw, y = count)) +
  geom_point(aes(color = factor(count)), size = 3) + # Punkte für die tatsächlichen Unfälle
  geom_line(aes(x = btw, y = prob_no_accidents * max(count)), color = "blue", size = 1) + # Poisson-Kurve
  labs(
    title = "Poisson-Modell: Wahrscheinlichkeiten für keine Unfälle",
    x = "Zentralitätsmaß",
    y = "Anzahl der Unfälle / Wahrscheinlichkeit für keine Unfälle",
    subtitle = "Blaue Linie: Wahrscheinlichkeit für keine Unfälle"
  ) +
  theme_minimal() 

ggplot(accident_per_joined_edge, aes(x = btw, y = prob_no_accidents)) +
  geom_point(aes(color = factor(count)), size = 3) + 
  geom_line(aes(x = btw, y = prob_no_accidents), color = "blue", size = 1) +
  labs(
    title = "Wahrscheinlichkeit für keine Unfälle auf Basis des Zentralitätsmaßes",
    x = "Zentralitätsmaß",
    y = "Wahrscheinlichkeit für keine Unfälle",
    subtitle = "Poisson-Modell basierend auf Zentralitätsmaß"
  ) +
  theme_minimal()

ggplot(accident_per_joined_edge, aes(x = count)) +
  geom_bar(stat = "count", fill = "lightblue", color = "black") +
  labs(
    title = "Häufigkeit von Unfällen auf verschiedenen Straßenabschnitten",
...
ggplot(accident_per_joined_edge) +
  geom_sf(aes(color = prob_no_accidents), size = 2) +  # Straßenabschnitte einfärben
  scale_color_viridis_c(option = "C", name = "Wahrscheinlichkeit für\nkeinen Unfall") +  # Farbskala
  labs(
    title = "Färbung der Straßenabschnitte nach Wahrscheinlichkeit für keine Unfälle",
    subtitle = "Poisson-Modell basierend auf Zentralitätsmaß"
  ) +
  theme_minimal() +
