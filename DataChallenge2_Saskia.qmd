...
The bicycle network constists of 24191 nodes and 29694 edges. It is undirected. The attributes of the edges are the name of the street, information about wether it is a bridge or a tunnel or neither, as well as the level of the highway. The geomertry is linestring. also, there are information about the from and to points, and the osm ID. The vertices are points without further attributes. The network is largly planar and does not contain self loops. It is not weighted.
...
trees <- st_within(edge_buffer, baumkataster_shp)
...
```{r}
accident_points <- st_within(edge_buffer,bike_accidents)
