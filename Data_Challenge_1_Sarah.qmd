...
As the smaller clusters have been excluded based on the method of KDE and the corresponding parameters chosen, only one surface remains which corresponds to a widened smoothed version of the major clusters in Task 9. This has lead to the data of both years being more similar which also reflects in the Jaccard Index value now being at 0.9. 
...
The commonalities include big surfaces spanning from the city center to Oerlikon and Altstetten, although the results from the KDE are wider and more smoothed. Another difference is that the KDE with the parameters we used only lead to one area per year instead of several clusters.

As the test for normal distribution of our data showed that our data are not normally distributed (we did not expect the data to be normally distributed) we first wanted to use plug-in bandwidth. But as we failed to export the KDS surface after 2.5h of struggling with the obscure "KDE" class of the output of the Hpi function we decided to resort to functions included in the adeHabitatHR package. Although the method tends to oversmooth with data, which is not normally distributed, we chose HREF as method instead of LSCV because we would rather have a slightly oversmoothed result instead of one containing every outlier. We chose to use the 90% range for both to reflect the percentage of points which was included in clusters back in Task 9. Looking at the HREF results I would recommend the plug-in bandwidth for this type of data because it did not oversmooth as much. But as a next best alternative HREF performed acceptable considering that our points were not normally distributed.
